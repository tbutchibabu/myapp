...
# ─── **Replace this block** with your exact mapping from the uploaded code ─────
TURBINE_MAP = {
    'DB91012': 'T01',
    'DB91010': 'T02',
    'DB91004': 'T03',
...
def get_data():
    ...
    data = { code: { agg: { t: [] for t in sel_t } for agg in aggs } for code in sel_codes }
    ...
    for turb in sel_t:
        ...
        for day in days:
            prefix = f"{GCS_PREFIX}/10Min/"
            blobs = _storage_client.list_blobs(GCS_BUCKET_NAME, prefix=prefix)
            for blob in blobs:
...
def get_dgr():
    ...
    for t in turbines:
        ...
        for day in days:
            ...
            prefix = f"{GCS_PREFIX}/Statistics/"
            blobs = _storage_client.list_blobs(GCS_BUCKET_NAME, prefix=prefix)
            for blob in blobs:
                fname = os.path.basename(blob.name)
                if not fname.lower().endswith(".zip"):
                    continue
                if code_key not in fname or dd not in fname:
                    continue
